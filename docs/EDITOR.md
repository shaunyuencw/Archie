...
Select a box to expose its resize corners and edges. Resizing from the top or left saves its changed position as well as its dimensions; zone contents keep their positions when the boundary moves. Right-click a box for **Bring to front**, **Bring forward**, **Send backward** or **Send to back**. Layer order is saved per view and can be undone. Zones remain containers behind their own contents.
